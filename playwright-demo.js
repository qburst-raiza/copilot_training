...
async function openPlaywrightDocs(options = {}) {
  ...
  try {
    ...
    // Assertion: Verify the main heading contains readable text
    if (!primaryHeading) {
      throw new Error('Assertion failed: Expected the first h1 heading to contain non-empty text');
    }
    console.log(`✓ Assertion passed: Main h1 heading contains text ("${primaryHeading.substring(0, 60)}")`);
